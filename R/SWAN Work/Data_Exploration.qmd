...
reduced_data <- data %>%
                  select(SWANID, STATUS0, SPBMDT0, HPBMDT0, OJCALC0, SUPCA0, DTTCALC0, ALLCALC0, AGE0, RACE, INCOME0)
```
...
spine_lm_base <- lm(SPBMDT0 ~ STATUS0 + OJCALC0 + SUPCA0 + SUPCA0 + DTTCALC0 + ALLCALC0, data = clean_data)
summary(spine_lm_base)

hip_lm_base <- lm(HPBMDT0 ~ STATUS0 + OJCALC0 + SUPCA0 + SUPCA0 + DTTCALC0 + ALLCALC0, data = clean_data)
summary(hip_lm_base)
```
...
spine_lm_demo <- lm(SPBMDT0 ~ STATUS0 + OJCALC0 + SUPCA0 + SUPCA0 + DTTCALC0 + ALLCALC0 + AGE0 + ETHNICITY + INCOME0, data = clean_data)
summary(spine_lm_demo)

hip_lm_demo <- lm(HPBMDT0 ~ STATUS0 + OJCALC0 + SUPCA0 + SUPCA0 + DTTCALC0 + ALLCALC0 + AGE0 + ETHNICITY + INCOME0, data = clean_data)
summary(hip_lm_demo)
```
